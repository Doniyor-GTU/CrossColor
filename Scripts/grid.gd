...
func add_coin_label():
	if coins_array.size() > 0:
		# Assumed that y_start > TopUi_height
		var TopUi_height = get_node("../ColorRect/TopUi").rect_size.y
		var difference = y_start - TopUi_height
...
#		elif n_x_n == 6:
#			space = 20
		coin_label.set_global_position(Vector2(screen_size_x / 2 - coin_label.rect_size.x, y_start - 20 - space))
		...
		if is_idx_in_grid(grid_pos.x, grid_pos.y) and is_started and not is_shuffling and not is_win:
			...
			if is_win():
				...
				if (current_level % 10) == 0:
					get_node("../CanvasLayer/GiftScreen").screen_in()
					GameDataManager.game_data["level"] = current_level + 1
					GameDataManager.save_game()
				else:
#					emit_signal("show_interstitial_add")
					GameDataManager.game_data["level"] = current_level + 1
					GameDataManager.save_game()
...
#	AddManager.show_reward_video("NextLevelButton")
	var current_level = get_parent().level
	GameDataManager.game_data["level"] = current_level + 1
	GameDataManager.save_game()
	if (str(current_level + 1) in LoadedScreens.levels):
...
func _on_ShuffleButton_pressed():
	if not is_shuffling and not is_win and is_started:
		shuffle_grid()
#		AddManager.show_reward_video("ShuffleButton")


#func _on_GameBoard_shuffle_grid():
#	if not is_shuffling and not is_win and is_started:
#		shuffle_grid()
